...
- **Art providers directory:** `apps/stage-tamagotchi/src/main/services/airi/widgets/providers/`
  ...
  - `comfyui.ts`, `nanobanana.ts`, `replicate.ts` — provider implementations registered in `artistry-bridge.ts:9-11,52-55`.
...
- **`generateHeadless` dedupes by JSON fingerprint** of `{ prompt, model, provider, options, globals }` (`artistry-bridge.ts:71-88`), hashing `globals.image` (sha256) to avoid megabyte keys. Two identical in-flight calls share one promise — but the dedup map is cleared in `finally` (line 205), so dedup only applies while the job is running.
- **Widget auto-generate triggers on `status: 'generating'`** (`artistry-bridge.ts:244`). If you only want to render an already-generated image, pass `status: 'done'` plus `imageUrl`; do not omit `status` from a `comfy`/`artistry` `update`, or `executeWidgetAction` will auto-set `status: 'generating'` (`builtin/widgets.ts:169-176`) and kick off a new run.
