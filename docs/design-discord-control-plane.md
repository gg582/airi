...
### 5.3 Deployment Session Selector

When clicking **`[🚀 Deploy Character to Cloudflare Edge]`**, a deployment modal prompts the user to select the target chat session:
- **Active Session (`main-session`)**: Deploys the character prompt and rolling context from the active GUI session.
- **Existing Session**: Selects another historical character session.
- **New Dedicated Relay Session**: Instantiates a fresh, isolated session dedicated specifically to Cloud Relay interactions.

### 5.4 Visual Memory Review & Reconciliation Modal

When clicking **`[Sync Memories ↓]`** on a deployed instance card, AIRI opens a **Reconciliation & Merge Modal**:
...
│                                                                        │
│ [x] 💬 10:42 AM — [dasilva333]: "Hey AIRI, remind me to check the GPU" │
│ [x] 🤖 10:42 AM — [AIRI]: "Got it! I logged that in your memory."       │
│ [ ] 💬 11:15 AM — [GuestUser]: "What's the weather in Tokyo?"          │
│ [x] 🧠 Fact Learned — "User loves tea and works on Electron apps"     │
│                                                                        │
│ [Select All] [Select Only Owner (dasilva333)]   [Cancel]  [Merge N Items]│
└────────────────────────────────────────────────────────────────────────┘
```
