...
## 5. Next Phased Experiments

With **Phase 0 and Phase 1 completed**, the harness is ready for:

### Phase 2: Echo Chip Multi-Word Theme Tag Synthesis (`02-echo-chip-tagger.ts`)
* **Goal**: Feed conversation slices into state-merged RWKV and verify multi-word theme tag output without rambling.
...
* **Goal**: Stream raw OS window titles and OCR screen telemetry into RWKV state and test constrained logit decoding (`PROMOTE` | `IGNORE`).
