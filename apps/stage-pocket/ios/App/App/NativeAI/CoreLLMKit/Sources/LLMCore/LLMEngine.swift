...
extension LLMEngineError: LocalizedError {
    public var errorDescription: String? {
        switch self {
        case .notLoaded:
            return "Model is not loaded."
        case .modelNotFound(let path):
            return "Model bundle manifest or file not found at '\(path)'. Please download the full model bundle first."
        case .incompatibleBundle(let reason):
            return "Incompatible model bundle: \(reason)"
        case .drafterUnavailable:
            return "Speculative drafter is unavailable."
        case .generationFailed(let reason):
            return "Generation failed: \(reason)"
        case .contextOverflow(let promptTokens, let contextLength):
            return "Context overflow: prompt has \(promptTokens) tokens but context length is \(contextLength)."
        }
    }
}
