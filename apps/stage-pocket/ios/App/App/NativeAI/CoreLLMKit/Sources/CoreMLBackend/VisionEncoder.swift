...
enum VLMPrompt {
    ...
    static func flatIDs(
        ...
        var ids: [Int] = [bos, turnStart]
        ids.append(contentsOf: userTokens)
        ids.append(boi)
        ids.append(contentsOf: Array(repeating: MultimodalSlot.imagePlaceholderID, count: imageRows))
        ids.append(eoi)
        ids.append(contentsOf: questionTokens)
        ids.append(contentsOf: [turnEnd, newline, turnStart])
        ids.append(contentsOf: modelTokens)
        return ids
    }
