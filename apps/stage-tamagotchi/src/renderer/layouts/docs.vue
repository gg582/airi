...
<template>
  <div class="relative h-full w-full overflow-hidden bg-black text-white">
    ...
    <div class="relative z-1 flex h-full w-full flex-col backdrop-blur-sm">
      <!-- Top Navigation Tabs (Glass) -->
      <div
        class="flex shrink-0 items-center gap-2 border-b border-white/10 bg-black/40 px-6 py-4 backdrop-blur-xl"
      >
        <div class="mr-6 flex items-center gap-3">
          <div class="h-8 w-8 flex items-center justify-center rounded-xl bg-gradient-to-br from-sky-400 to-purple-600 shadow-lg">
            <div i-solar:book-open-bold class="text-white text-lg" />
          </div>
          <span class="text-lg font-black tracking-tight text-white/90">Documentation</span>
        </div>
        ...
        <button
          ...
          :key="section.id"
          class="group relative flex items-center gap-2 rounded-full px-5 py-2 transition-all duration-300"
          :class="[
        ...
        >
          <div :class="[section.icon, 'text-lg transition-transform group-hover:scale-110']" />
          <span class="text-xs font-bold uppercase tracking-[0.2em]">{{ t(section.titleKey) }}</span>
          ...
          <div 
            ...
            class="absolute -bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-sky-400 shadow-[0_0_8px_rgba(56,189,248,0.8)]"
          />
        </button>
      </div>
      ...
      <div class="flex min-h-0 flex-1">
        ...
        <div
          ...
          :enter="{ opacity: 1, x: 0 }"
          :duration="600"
          class="w-72 flex flex-col border-r border-white/5 bg-black/20 p-4 shadow-xl backdrop-blur-md"
        >
          <div class="mb-4 flex items-center gap-2 px-2 py-1">
            <div class="h-px flex-1 bg-white/10" />
            <span class="text-[10px] font-black uppercase tracking-[0.3em] text-sky-400/60">Navigation</span>
          ...
          <div class="flex-1 overflow-y-auto scrollbar-none pr-1">
            <div v-for="group in sidebarItems" :key="group.text" class="mb-8">
              <div class="mb-3 px-3 text-[10px] font-black uppercase tracking-[0.25em] text-white/30">
                {{ group.text }}
              </div>
              ...
              <div class="flex flex-col gap-1.5">
                <template v-for="item in group.items" :key="item.text">
                  ...
                  <div v-if="item.items" class="mb-1">
                    <div class="flex items-center gap-2 px-3 py-1 mb-1 text-[11px] font-bold text-white/50">
                      <div class="h-1.5 w-1.5 rounded-full bg-purple-500/40" />
                      {{ item.text }}
                    </div>
                    <div class="ml-3.5 border-l border-white/5 pl-3 flex flex-col gap-1">
                      <button
        ...
        <div class="relative flex flex-1 flex-col overflow-hidden bg-black/30">
          <div class="pointer-events-none absolute inset-0 bg-gradient-to-br from-sky-500/5 via-transparent to-purple-500/5" />
          
          <main class="relative z-1 h-full overflow-y-auto p-8 lg:p-12 scrollbar-none">
            <div class="mx-auto max-w-4xl">
              ...
              >
                ...
                <span class="text-[10px] font-black uppercase tracking-[0.4em] text-sky-400">
              ...
              <div 
                class="fixed bottom-8 right-8 z-50 flex items-center gap-1 rounded-2xl border border-white/10 bg-black/60 p-1 backdrop-blur-2xl shadow-2xl"
              >
                <button
                  v-for="l in ['en', 'zh-Hans', 'ja']"
                  :key="l"
                  class="rounded-xl px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all"
                  :class="[
                    locale === l 
