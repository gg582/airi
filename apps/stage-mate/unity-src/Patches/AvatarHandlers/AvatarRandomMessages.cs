...
public class AvatarRandomMessages : MonoBehaviour
{
    ...
    /// <summary>
    /// Displays a real-time custom speech caption from AIRI using MATE Engine's authentic native bubble system.
    /// </summary>
    public void ShowCustomCaption(string text)
    {
        ...
        {
            Debug.LogWarning("[AvatarRandomMessages] chatContainer is null!");
            return;
        }

        // Ensure chatContainer hierarchy is active
        Transform p = chatContainer;
        while (p != null)
        {
