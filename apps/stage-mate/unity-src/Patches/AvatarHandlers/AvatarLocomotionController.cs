...
public sealed class AvatarLocomotionController : MonoBehaviour
{
    ...
    [Range(0f, 60f)] public float Randomizer = 3f;
    [Range(10f, 4000f)] public float MinWalkCycle = 150f;
    [Range(10f, 4000f)] public float MaxWalkCycle = 350f;
    ...
    void StartWalk(bool forced)
    {
        ...
        Debug.Log($"[Locomotion] Started walk: dir={_dir}, pixels={_remainingPixels}");
    }
    ...
    void StepWalk()
    {
        ...
        if (uniwinc != null)
        {
            ...
            float actualMoved = Mathf.Abs(clampedX - curPos.x);
            _remainingPixels -= actualMoved;

            uniwinc.windowPosition = new Vector2(clampedX, Mathf.Max(0f, curPos.y));

            if (actualMoved <= 0.01f)
            {
                _remainingPixels = 0f;
                _forcedNextDir = -_dir;
            }
    ...
    void ScheduleNextDecision(bool immediate)
    {
        ...
        _nextDecisionTime = t + UnityEngine.Random.Range(baseDelay, baseDelay * 1.5f);
        UnityEngine.Debug.Log($"[Locomotion] Scheduled next walk in ~{baseDelay:F1}s (at t={_nextDecisionTime:F1})");
    }
    ...
    struct BlockingInfo
    {
        ...
        public float effectiveMinGlobalX;
        public float effectiveMaxGlobalX;
        public float edgeThresholdGlobal;
    ...
    bool TryGetBlockingInfo(out BlockingInfo bi)
    {
        ...
        for (int i = 0; i < _boundsRenderers.Length; i++)
        {
            ...
            FillCorners(b, _boundsCorners);

            for (int c = 0; c < 8; c++)
            {
                Vector3 sp = cam.WorldToScreenPoint(_boundsCorners[c]);
                if (sp.z < 0f) continue;
        ...
        if (!hasValidRenderer || minX >= maxX)
            return false;

        float insetL = Mathf.Max(0f, BoundsInsetLeft);
        float insetR = Mathf.Max(0f, BoundsInsetRight);

        float effMinX = minX + insetL;
        float effMaxX = maxX - insetR;
        ...
        bi.effectiveMinXUnity = effMinX;
        bi.effectiveMaxXUnity = effMaxX;

        float thrUnity = Mathf.Max(0f, EdgeThresholdUnityPixels);
        bi.edgeThresholdUnity = thrUnity;
        ...
        int mLeft = 0;
        int mRight = Display.main.systemWidth;

#if (UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN)
        if (!TryGetMonitorBounds(_hwnd, out mLeft, out mRight))
        {
            int vx = GetSystemMetrics(SM_XVIRTUALSCREEN);
            int vw = GetSystemMetrics(SM_CXVIRTUALSCREEN);
            mLeft = vx;
        ...
        bi.monitorLeft = mLeft;
        bi.monitorRight = mRight;

        float effMinGlobal = winLeft + effMinX;
        float effMaxGlobal = winLeft + effMaxX;

        bi.effectiveMinGlobalX = effMinGlobal;
        bi.effectiveMaxGlobalX = effMaxGlobal;
        bi.edgeThresholdGlobal = thrUnity;

        int minWinX = Mathf.RoundToInt(mLeft - effMinX);
        int maxWinX = Mathf.RoundToInt(mRight - effMaxX);

        if (maxWinX < minWinX)
            maxWinX = minWinX;

        bi.minWindowX = minWinX;
        bi.maxWindowX = maxWinX;

        return true;
    }

    static void FillCorners(Bounds b, Vector3[] out8)
    {
        Vector3 c = b.center;
        Vector3 e = b.extents;

        out8[0] = new Vector3(c.x - e.x, c.y - e.y, c.z - e.z);
        out8[1] = new Vector3(c.x + e.x, c.y - e.y, c.z - e.z);
        out8[2] = new Vector3(c.x - e.x, c.y + e.y, c.z - e.z);
        out8[3] = new Vector3(c.x + e.x, c.y + e.y, c.z - e.z);
        out8[4] = new Vector3(c.x - e.x, c.y - e.y, c.z + e.z);
        out8[5] = new Vector3(c.x + e.x, c.y - e.y, c.z + e.z);
        out8[6] = new Vector3(c.x - e.x, c.y + e.y, c.z + e.z);
        out8[7] = new Vector3(c.x + e.x, c.y + e.y, c.z + e.z);
    }

#if (UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN)
    static bool TryGetMonitorBounds(IntPtr hWnd, out int monitorLeft, out int monitorRight)
    {
        monitorLeft = 0;
        monitorRight = 0;
        ...
        mi.cbSize = (int)Marshal.SizeOf(typeof(MONITORINFO));

        if (!GetMonitorInfo(hMon, ref mi)) return false;

        monitorLeft = mi.rcWork.Left;
        monitorRight = mi.rcWork.Right;
        return true;
    }
#endif
}
