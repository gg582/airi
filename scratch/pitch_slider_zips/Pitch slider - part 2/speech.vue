        ...
        <div flex="~ col gap-4">
          ...
          <div flex="~ col gap-4">
            ...
            <div class="mt-4 border-t border-neutral-100 pt-4 dark:border-neutral-800">
              <h3 class="mb-3 flex items-center gap-1.5 text-sm text-neutral-800 font-semibold dark:text-neutral-200">
                <span class="i-solar:tuning-square-2-bold-duotone text-lg text-primary-500" />
                Creative Voice Effects
              </h3>
              <div flex="~ col gap-4" class="pl-2">
                <FieldRange
                  v-model="voiceEffectRadio"
                  label="Radio Filter"
                  description="Walkie-talkie / vintage megaphone filter"
                  :min="0" :max="100" :step="1"
                  :format-value="value => `${value}%`"
                />
                <FieldRange
                  v-model="voiceEffectRobot"
                  label="Robotic Droid"
                  description="Metallic cybernetic resonator resonance mix"
                  :min="0" :max="100" :step="1"
                  :format-value="value => `${value}%`"
                />
                <FieldRange
                  v-model="voiceEffectReverb"
                  label="Spacious Reverb"
                  description="Cathedral space echo and decay intensity"
                  :min="0" :max="100" :step="1"
                  :format-value="value => `${value}%`"
                />
                <FieldRange
                  v-model="voiceEffectAsmr"
                  label="ASMR Whisper"
                  description="Soft warm compression and high roll-off mix"
                  :min="0" :max="100" :step="1"
                  :format-value="value => `${value}%`"
                />
                <FieldRange
                  v-model="voiceEffectSpatial"
                  label="Stereo Panning"
                  description="Manual left-to-right stereo voice placement"
                  :min="-100" :max="100" :step="1"
                  :format-value="value => value === 0 ? 'Center' : value < 0 ? `Left ${Math.abs(value)}%` : `Right ${value}%`"
                />
              </div>
            </div>
