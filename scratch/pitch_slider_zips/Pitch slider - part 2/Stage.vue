...
const { ssmlEnabled, activeSpeechProvider, activeSpeechModel, activeSpeechVoice, pitch, voiceEffectRadio, voiceEffectRobot, voiceEffectReverb, voiceEffectSpatial, voiceEffectAsmr } = storeToRefs(speechStore)
...
async function playFunction(item: Parameters<Parameters<typeof createPlaybackManager<AudioBuffer>>[0]['play']>[0], signal: AbortSignal): Promise<void> {
  ...
  // Apply pitch & rate adjustments via playbackRate for providers that do not support it natively (like Kokoro Local)
  ...
  // Connect the effects chain series
  let lastNode: AudioNode = source

  // 1. ASMR Warmth / Whisper Effect (Lowpass filter + Compression)
  const asmrVal = voiceEffectAsmr.value || 0
  if (asmrVal > 0) {
  ...
  const radioVal = voiceEffectRadio.value || 0
  if (radioVal > 0) {
  ...
  const robotVal = voiceEffectRobot.value || 0
  if (robotVal > 0) {
  ...
  const reverbVal = voiceEffectReverb.value || 0
  if (reverbVal > 0) {
    const delayNode = audioContext.createDelay()
  ...
  const spatialVal = voiceEffectSpatial.value || 0
