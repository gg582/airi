...
export const useSpeechStore = defineStore('speech', () => {
  ...
  // Voice Effects Sliders (0 to 100%)
  const voiceEffectRadio = useLocalStorageManualReset<number>('settings/speech/effect-radio', 0)
  const voiceEffectRobot = useLocalStorageManualReset<number>('settings/speech/effect-robot', 0)
  const voiceEffectReverb = useLocalStorageManualReset<number>('settings/speech/effect-reverb', 0)
  const voiceEffectSpatial = useLocalStorageManualReset<number>('settings/speech/effect-spatial', 0) // -100 to 100 (Pan)
  const voiceEffectAsmr = useLocalStorageManualReset<number>('settings/speech/effect-asmr', 0)
  ...
  function resetState() {
    ...
    pitch.reset()
    rate.reset()
    voiceEffectRadio.reset()
    voiceEffectRobot.reset()
    voiceEffectReverb.reset()
    voiceEffectSpatial.reset()
    voiceEffectAsmr.reset()
    ssmlEnabled.reset()
