...
def build_demo(args: argparse.Namespace):
    ...
    with gr.Blocks(title="MOSS-TTS Demo") as demo:
        gr.Markdown(
            """
            <div class="app-card">
    ...
    demo._moss_custom_css = custom_css
    return demo
...
def main():
    ...
    if "css" in launch_parameters:
        launch_kwargs["css"] = getattr(demo, "_moss_custom_css", None)
    if "ssr_mode" in launch_parameters:
        launch_kwargs["ssr_mode"] = False

    demo.queue(max_size=16, default_concurrency_limit=1).launch(**launch_kwargs)
