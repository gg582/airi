...
const require_buffer = __commonJS({
  'node_modules/buffer/index.js': function (exports) {
    'use strict'
...
let module_empty_default
const init_module_empty = __esm({
  'shims/module-empty.js': function () {
...
const import_buffer = __toESM(require_buffer(), 1)
...
const createSentencePieceModule = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    var PATH = { isAbs: path => path.charAt(0) === '/', splitPath: (filename) => {
      ...
      const isAbsolute = PATH.isAbs(path); const trailingSlash = path.substr(-1) === '/'
    ...
    const initRandomFill = () => {
      if (typeof crypto == 'object' && typeof crypto.getRandomValues == 'function') {
        ...
        try {
          const crypto_module = require2('crypto')
    ...
    function intArrayFromString(stringy, dontAddNull, length) {
      const len = lengthBytesUTF8(stringy) + 1
    ...
    const asyncLoad = (url, onload, onerror, noRunDep) => {
      const dep = getUniqueRunDependency(`al ${url}`)
    ...
    const FS_modeStringToFlags = (str) => {
      const flagModes = { 'r': 0, 'r+': 2, 'w': 512 | 64 | 1, 'w+': 512 | 64 | 2, 'a': 1024 | 64 | 1, 'a+': 1024 | 64 | 2 }
    ...
    const __embind_register_bool = (rawType, name, trueValue, falseValue) => {
      ...
      registerType(rawType, { name, fromWireType(wt) {
        return !!wt
    ...
    const shallowCopyInternalPointer = o => ({ count: o.count, deleteScheduled: o.deleteScheduled, preservePointerOnDelete: o.preservePointerOnDelete, ptr: o.ptr, ptrType: o.ptrType, smartPtr: o.smartPtr, smartPtrType: o.smartPtrType })
    ...
    function createJsInvoker(argTypes, isClassMethodFunc, returns, isAsync) {
      ...
      const argCount = argTypes.length - 2
      let argsList = []
...
`
    ...
    const __embind_register_class_function = (rawClassType, methodName, argCount, rawArgTypesAddr, invokerSignature, rawInvoker, context, isPureVirtual, isAsync, isNonnullReturn) => {
      const rawArgTypes = heap32VectorToArray(argCount, rawArgTypesAddr)
    ...
    const EmValOptionalType = Object.assign({ optional: true }, EmValType)
    const __embind_register_optional = (rawOptionalType, rawType) => {
      registerType(rawOptionalType, EmValOptionalType)
    }
    const __embind_register_smart_ptr = (rawType, rawPointeeType, name, sharingPolicy, getPointeeSignature, rawGetPointee, constructorSignature, rawConstructor, shareSignature, rawShare, destructorSignature, rawDestructor) => {
      name = readLatin1String(name)
      ...
      whenDependentTypesAreResolved([rawType], [rawPointeeType], (pointeeType) => {
        ...
        const registeredPointer = new RegisteredPointer(name, pointeeType.registeredClass, false, false, true, pointeeType, sharingPolicy, rawGetPointee, rawConstructor, rawShare, rawDestructor)
...
`
      ...
      const invoker = kind === 1 ? 'new func' : 'func.call'
...
const SentencePieceProcessor = class {
  uuidv4() {
...
globalThis.SentencePieceProcessor = SentencePieceProcessor
